...
describe('CreateCityDto', () => {
  ...
  test('should create a valid DTO with correct values', () => {
    ...
    // Creación del DTO
    const [error, createCityDto] = CreateCityDto.create(cityData);
    
    // Verificaciones
    expect(error).toBeUndefined();
    expect(createCityDto).toBeInstanceOf(CreateCityDto);
    
    // Verificar valores correctos y transformaciones
    expect(createCityDto?.name).toBe('buenos aires'); // debe estar en minúsculas
    expect(createCityDto?.description).toBe('capital de argentina'); // debe estar en minúsculas
    expect(createCityDto?.isActive).toBe(true);
  });
  ...
  test('should set isActive to true by default if not provided', () => {
    ...
    expect(error).toBeUndefined();
    expect(createCityDto).toBeInstanceOf(CreateCityDto);
    expect(createCityDto?.isActive).toBe(true); // valor por defecto
  });
  ...
  test('should return error if isActive is not a boolean', () => {
    ...
    // Creación del DTO
    const [error, createCityDto] = CreateCityDto.create(cityData as any);
