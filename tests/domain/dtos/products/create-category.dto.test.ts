...
import { CreateCategoryDto } from '../../../../src/domain/dtos/products/create-category';
...
describe('CreateCategoryDto', () => {
  ...
  test('should create a valid DTO with correct values', () => {
    ...
    expect(createCategoryDto?.name).toBe('test category'); // debe estar en minúsculas
    expect(createCategoryDto?.description).toBe('test category description'); // debe estar en minúsculas
    expect(createCategoryDto?.isActive).toBe(true);
  });
  ...
  test('should handle non-boolean isActive values', () => {
    ...
    // Creación del DTO
    const [error, createCategoryDto] = CreateCategoryDto.create(categoryData as any);
  ...
  test('should convert name and description to lowercase', () => {
    ...
    expect(error).toBeUndefined();
    expect(createCategoryDto).toBeInstanceOf(CreateCategoryDto);
    expect(createCategoryDto?.name).toBe('test category');
    expect(createCategoryDto?.description).toBe('test category description');
  });
});