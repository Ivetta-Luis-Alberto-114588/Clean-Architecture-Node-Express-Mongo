...
describe('NeighborhoodEntity', () => {
    ...
    test('should create a NeighborhoodEntity instance with all properties', () => {
        ...
        const isActive = true;
        // Act
        const neighborhood = new neighborhood_1.NeighborhoodEntity(id, name, description, mockCity, isActive);
        // Assert
        expect(neighborhood).toBeInstanceOf(neighborhood_1.NeighborhoodEntity);
    ...
    test('should work with different City instances', () => {
        // Arrange
        const anotherCity = new citiy_1.CityEntity(99, 'Another City', 'Another City Description', false);
        const id = 6;
        const name = 'Multi-city Neighborhood';
        const description = 'Neighborhood testing multiple cities';
        const isActive = true;
        // Act
        const neighborhood = new neighborhood_1.NeighborhoodEntity(id, name, description, anotherCity, isActive);
        // Assert
    ...
    test('should accept zero or negative ID values', () => {
        // Arrange - ID cero
        const idZero = 0;
        const neighborhood1 = new neighborhood_1.NeighborhoodEntity(idZero, 'Zero ID', 'Zero ID Description', mockCity, true);
        // ID negativo
        const idNegative = -7;
        const neighborhood2 = new neighborhood_1.NeighborhoodEntity(idNegative, 'Negative ID', 'Negative ID Description', mockCity, true);
        // Assert
