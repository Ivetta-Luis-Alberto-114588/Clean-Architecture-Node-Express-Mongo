...
import { SearchCustomersUseCaseImpl } from "../../domain/use-cases/customers/search-customers.use-case";
...
export class CustomerController {
    ...
    searchCustomers = (req: Request, res: Response): void => {
        const [error, searchCustomersDto] = SearchCustomersDto.create(req.query);

        if (error) {
            res.status(400).json({ error });
            console.log("Error en controller.customer.searchCustomers", error);
            return;
        }
